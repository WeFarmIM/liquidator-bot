...
export class TestGenerator extends TaskExecutor {
  ...
  resetAllPrice = async () => {
    for (let i = 0; i < 2; ++i) {
      ...
      const price = await getPrice(tokenName, this.owner, this.web3Wrapper.getWeb3(), this.address);
      logger.debug({
  ...
  withdrawDeposit = async (account: string) => {
    await withdrawAll("DAI", this.gasPriceGetter.getLatestPrice(), account, this.web3Wrapper.getWeb3(), this.address);
  };
  ...
  checkStatus = async () => {
    for (let account of this.accounts) {
      let borrowDAIAmt = (
        await borrowBalance("DAI", account, this.owner, this.web3Wrapper.getWeb3(), this.address)
      ).toString();

      let depositDAIAmt = (
        await depositBalance("DAI", account, this.owner, this.web3Wrapper.getWeb3(), this.address)
      ).toString();

      let borrowUSDCAmt = (
        await borrowBalance("USDC", account, this.owner, this.web3Wrapper.getWeb3(), this.address)
      ).toString();

      let depositUSDCAmt = (
        await depositBalance("USDC", account, this.owner, this.web3Wrapper.getWeb3(), this.address)
      ).toString();
