...
export class Web3Wrapper {
  ...
  public setWeb3 = (type: string, mnemonic: string, privateKey: string, accessNode: string) => {
    if (type === "ganache") {
      ...
      const provider = new HDWalletProvider(/* your mnemonic */ mnemonic, /* your infura access node*/ accessNode);
      this.web3.setProvider(provider);
    } else {
      const provider = new PrivateKeyProvider(privateKey, accessNode);
      this.web3.setProvider(provider);
    }
  };
